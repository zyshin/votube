	...
	if (supportsVideo) {
		...
		if (document.addEventListener) {
			...
			var createMenuItem = function(id, lang, label) {
				...
				button.addEventListener('click', function(e) {
					...
					for (var i = 0; i < video.textTracks.length; i++) {
						...
						if (video.textTracks[i].label == lang) {
							video.textTracks[i].mode = 'showing';
							this.setAttribute('data-state', 'active');
							subtitles.textContent = lang;
						}
						else {
							video.textTracks[i].mode = 'hidden';
						}
					}
					if (!lang) subtitles.textContent = 'CC';
			...
			if (video.textTracks) {
				...
				for (var i = 0; i < video.textTracks.length; i++) {
					subtitlesMenu.appendChild(createMenuItem('subtitles-' + video.textTracks[i].label, video.textTracks[i].label, video.textTracks[i].label));
				}
